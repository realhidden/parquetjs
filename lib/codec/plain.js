...
function encodeValues_BOOLEAN(values) {
  let buf = Buffer.allocUnsafe(Math.ceil(values.length / 8));
  buf.fill(0);
...
function encodeValues_INT32(values) {
  let buf = Buffer.allocUnsafe(4 * values.length);
  for (let i = 0; i < values.length; i++) {
...
function decodeValues_INT32(cursor, count) {
  ...
  for (let i = 0; i < count; ++i) {
    values[i] = cursor.buffer.readInt32LE(cursor.offset);
    cursor.offset += 4;
  }
...
function encodeValues_INT64(values) {
  let buf = Buffer.allocUnsafe(8 * values.length);
  for (let i = 0; i < values.length; i++) {
...
function decodeValues_INT64(cursor, count) {
  ...
  for (let i = 0; i < count; ++i) {
    values[i] = INT53.readInt64LE(cursor.buffer, cursor.offset);
    cursor.offset += 8;
  }
...
function encodeValues_INT96(values) {
  let buf = Buffer.allocUnsafe(12 * values.length);
...
function decodeValues_FLOAT(cursor, count) {
  ...
  for (let i = 0; i < count; ++i) {
    values[i] = cursor.buffer.readFloatLE(cursor.offset);
    cursor.offset += 4;
  }
...
function decodeValues_DOUBLE(cursor, count) {
  ...
  for (let i = 0; i < count; ++i) {
    values[i] = cursor.buffer.readDoubleLE(cursor.offset);
    cursor.offset += 8;
  }
...
function decodeValues_BYTE_ARRAY(cursor, count) {
  ...
  for (let i = 0; i < count; ++i) {
    ...
    cursor.offset += 4;
    values[i] = cursor.buffer.slice(cursor.offset, cursor.offset + len);
    cursor.offset += len;
  }
...
function decodeValues_FIXED_LEN_BYTE_ARRAY(cursor, count, opts) {
  ...
  for (let i = 0; i < count; ++i) {
    values[i] = cursor.buffer.slice(cursor.offset, cursor.offset + opts.typeLength);
