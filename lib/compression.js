...
// Lazily load zstd to avoid failing require - similar to lzo
...
/**
 * Lazyily load zstd, avoids potential failing require
 * unless there was an attempt to access it
 */
function get_zstd() {
  return require('fast-zstd');
}
...
function deflate_zstd(value) {
  return get_zstd().compress(value, 6);
}
...
function inflate_zstd(value) {
  return Buffer.from(get_zstd().decompress(value));
}
